...
- **Plugins Gradle en el monorepo**: `Galactifun` y `Bump` compilan en CI (`gradle_green`); `CustomItemGenerators`, `FastMachines` y `SlimefunTranslation` siguen **bloqueados** en el reactor completo (ver matriz).
...
| :--- | :--- | :--- |
| **CI Monorepo 1.21** | Verde en `1.21-latin` | Workflow unificado (`ci-monorepo-121.yml`); jobs Maven + `gradle_green`; no implica 86/86 en un solo comando. |
...
| **Slimefun Core + Dough** | Listo en job `foundation` | Baseline Paper 1.21.1 + Java 21. |
...
> CI principal en `1.21-latin`: **CI Monorepo 1.21** en verde (`ci-monorepo-121.yml`, jobs curados).
