...
public class RegistryAccessMock implements RegistryAccess
{
	...
	private static @NotNull BiMap<RegistryKey<?>, String> createClassToKeyConversions()
	{
		...
		for (RegistryKey<?> registryKey : getAllKeys())
		{
			...
			else if (registryKey == RegistryKey.GAME_RULE)
			{
				output.put(registryKey, "org.bukkit.GameRule");
			}
			else if (registryKey == RegistryKey.ZOMBIE_NAUTILUS_VARIANT)
			{
				output.put(registryKey, "org.bukkit.entity.ZombieNautilus$Variant");
			}
			else
			{
