...
| Bump | addon (Gradle) | Bloqueado (build) | Gradle monorepo | `sources/community-addons/Bump` | Errores masivos Java: APIs `MenuBlock`/`ItemGroup` legacy vs paquete Drake, menus Lombok, simbolos Bukkit renombrados (parte corregida con `port_paper_121`). |
